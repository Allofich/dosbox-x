		...
		do {    /* File name and extension */
			...
			if(!(attr&DOS_ATTR_DIRECTORY && (!strcmp(name, ".") || !strcmp(name, "..")))) {
				...
				if (dot2==NULL) {
					star=strchr(arg2,'*');
					if (strchr(arg2,'?')) {
						for (size_t i=0; i<(uselfn?LFN_NAMELENGTH:DOS_NAMELENGTH) && i<(star?(size_t)(star-arg2):strlen(arg2)); i++) {
					...
					*dot2='.';
					star=strchr(tname2,'*');
					if (strchr(tname2,'?')) {
						for (size_t i=0; i<(uselfn?LFN_NAMELENGTH:DOS_NAMELENGTH) && i<(star?(size_t)(star-tname2):strlen(tname2)); i++) {
					...
					if (dot1) {
						...
						star=strchr(text2,'*');
						if (strchr(text2,'?')) {
							for (size_t i=0; i<(uselfn?LFN_NAMELENGTH:DOS_NAMELENGTH) && i<(star?(size_t)(star-text2):strlen(text2)); i++) {
